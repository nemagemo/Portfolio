    ...
    <>
      ...
      <div className={`w-full p-3 sm:p-4 ${styles.cardContainer} relative overflow-hidden mb-6`}>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 relative z-10">
          ...
          <div className={`lg:col-span-3 flex flex-col justify-center lg:border-l lg:border-r lg:border-slate-100 lg:px-4`}>
             <h2 className="text-[10px] uppercase tracking-widest font-bold mb-1 text-slate-400">
               Zysk
             </h2>
             <div className={`text-xl sm:text-2xl font-bold mb-0.5 ${profitValue >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}>
                    ...
                    <SimpleStatBlock 
                        ...
                        value={(
                          <div className="flex items-baseline gap-1">
                            ...
                            <span className="text-base font-medium opacity-50 text-slate-500 border-slate-300 border-b border-dashed">
          ...
          <div className="flex items-center space-x-2">
             ...
             {portfolioType === 'PPK' && setShowPPKProjection && (
                <button
                  ...
                  className={`hidden md:flex items-center px-3 py-1.5 text-xs font-bold rounded-md transition-all border ${showPPKProjection ? styles.toggleProjectionActive : `bg-transparent text-slate-500 hover:text-slate-700 border-slate-200`}`}
                >
             ...
             {portfolioType === 'IKE' && setShowTaxComparison && (
                <button 
                  ...
                  className={`hidden md:flex items-center px-3 py-1.5 text-xs font-bold rounded-md transition-all border ${showTaxComparison ? 'bg-amber-50 text-amber-700 border-amber-200' : `bg-transparent text-slate-500 hover:text-slate-700 border-slate-200`}`}
      ...
      {portfolioType === 'IKE' && (
        <div className={`mb-6 p-6 ${styles.cardContainer}`}>
          ...
          <DrawdownChart data={drawdownData} themeMode={theme as any} />
        </div>
      )}
      ...
      {portfolioType === 'IKE' && (
        <div className="hidden md:grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                        <div className="flex border p-0.5 bg-white border-slate-200 rounded-md">
                            <button onClick={() => setDividendViewMode('Yearly')} className={`px-2 py-1 text-[10px] font-medium rounded ${dividendViewMode === 'Yearly' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>Rok</button>
                            <button onClick={() => setDividendViewMode('Quarterly')} className={`px-2 py-1 text-[10px] font-medium rounded ${dividendViewMode === 'Quarterly' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>Kwartał</button>
                        </div>
                        <div className={`p-2 rounded-lg ${styles.cardHeaderIconBg}`}>
                            <IconDividends className="w-5 h-5 text-emerald-600" />
                        </div>
                    </div>
                </div>
                <DividendChart data={dividendChartData} themeMode={theme as any} />
            </div>
            ...
            <div className={`${styles.cardContainer} p-6`}>
                ...
                <SectorAllocationChart data={sectorAllocationData} themeMode={theme as any} />
            </div>
        </div>
      )}
    </>

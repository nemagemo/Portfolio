...
export const TurtleDashboard: React.FC<TurtleDashboardProps> = ({ 
  ...
  // Calculate step and size inside component scope so it's accessible for custom tag positions
  const step = Math.max(1, Math.floor((containerWidth * 0.8) / 100));
  const size = 100 * step;

  // Spacing layout helper around the perimeter
  const getVisualPosition = (p: number) => {
    if (p < 100) {
      return { top: '-34px', left: `${p}%`, transform: 'translateX(-50%)' };
    } else if (p < 200) {
      return { top: `${p - 100}%`, left: 'calc(100% + 14px)', transform: 'translateY(-50%)' };
    } else if (p < 300) {
      return { top: 'calc(100% + 14px)', left: `${300 - p}%`, transform: 'translateX(-50%)' };
    } else {
      return { top: `${400 - p}%`, left: '-34px', transform: 'translateY(-50%)' };
    }
  };
  ...
  useEffect(() => {
    ...
    // Pre-calculate anchors and partition cells
    const N = turtles.length;
    const anchors = turtles.map((_, i) => {
      // Offset by half a step to center nicely on the sides
      const p = (i * (400 / N) + (200 / N)) % 400;
      let r = 0;
      let c = 0;
      if (p < 100) {
        r = 0;
        c = Math.min(99, Math.floor(p));
      } else if (p < 200) {
        r = Math.min(99, Math.floor(p - 100));
        c = 99;
      } else if (p < 300) {
        r = 99;
        c = Math.max(0, Math.min(99, Math.floor(299 - p)));
      } else {
        r = Math.max(0, Math.min(99, Math.floor(399 - p)));
        c = 0;
      }
      return { r, c };
    });

    // Helper to determine if a pixel belongs to the stylized ominous gray face in the center of the grid (symmetrical around center col 50)
    const isEvilFacePixel = (r: number, c: number): boolean => {
      const ec = Math.abs(c - 50); // distance from center line (0 to 50)

      // 1. Sleek, curvaceous demonic horns (extending upwards and outwards)
      if (ec >= 10 && ec <= 28) {
        const dx = ec - 10;
        // parabolic curve mapping ec to a peak at the edges
        const hornY = 28 - dx * 1.3 + 0.022 * dx * dx;
        const thickness = Math.max(1, 4.5 - dx * 0.16); // Tapering
        if (r >= hornY - thickness && r <= hornY + thickness) {
          return true;
        }
      }

      // 2. Forehead demonic glyph (inverted star triangle / central third-eye slit + anger frown wrinkles)
      // Central vertical third eye/split
      if (ec <= 1.5 && r >= 22 && r <= 27) {
        return true;
      }
      // Slanting forehead wrinkles (anger marks)
      if (r >= 22 && r <= 29 && ec >= 4 && ec <= 9) {
        const targetY = 21 + (ec - 4) * 0.9;
        if (Math.abs(r - targetY) < 1.2) {
          return true;
        }
      }

      // 3. Fierce, angry eyebrows (thick slanted down towards nose)
      if (ec >= 3 && ec <= 19) {
        const browY = 32 + (19 - ec) * 0.65;
        if (r >= browY && r < browY + 3.2) {
          return true;
        }
      }

      // 4. Menacing, highly slanted eye slits below eyebrows
      if (ec >= 5 && ec <= 17) {
        const eyeY = 40 + (17 - ec) * 0.5;
        const halfHeight = Math.max(0.8, 2.4 - Math.abs(ec - 11) * 0.25);
        if (r >= eyeY - halfHeight && r <= eyeY + halfHeight) {
          return true;
        }
      }

      // 5. Angular peak/nose cavity (demonic skull peak)
      if (r >= 49 && r <= 56) {
        const maxW = (r - 49) * 0.75;
        if (ec <= maxW) {
          return true;
        }
      }

      // 6. Jagged cheekbones and flared cheek armor plates
      if (ec >= 18 && ec <= 24 && r >= 48 && r <= 56) {
        const cheekY = 48 + (ec - 18) * 0.75;
        if (Math.abs(r - cheekY) < 1.8) {
          return true;
        }
      }

      // 7. Evil demonic grin with prominent sharp fangs (upper and lower interlocking teeth)
      if (ec <= 23) {
        const mouthY = 66 - (ec * ec * 0.038);
        
        // Lips / outer mouth structure
        if (r >= mouthY - 1.5 && r <= mouthY + 1.8) {
          return true;
        }

        // Pointy primary upper fangs hanging down
        if (ec >= 5 && ec <= 7 && r >= mouthY && r <= mouthY + 6) {
          if (r - mouthY <= 6.5 - Math.abs(ec - 6) * 3) {
            return true;
          }
        }
        // Lateral secondary upper fangs
        if (ec >= 13 && ec <= 15 && r >= mouthY && r <= mouthY + 5) {
          if (r - mouthY <= 5 - Math.abs(ec - 14) * 2.5) {
            return true;
          }
        }

        // Interlocking lower fangs pointing upwards
        if (ec >= 9 && ec <= 11 && r <= mouthY && r >= mouthY - 5.5) {
          if (mouthY - r <= 6 - Math.abs(ec - 10) * 3) {
            return true;
          }
        }

        // Small inner teeth ridges elsewhere
        if (r >= mouthY - 2.5 && r <= mouthY + 2.8) {
          if ((c % 3) !== 0) {
            return true;
          }
        }
      ...
      // 8. Chiseled chin spikes (multiple aggressive spikes at the bottom)
      if (r >= 74 && r <= 84 && ec <= 13) {
        const chinY = 74 + ec * 0.8;
        if (Math.abs(r - chinY) < 1.8) {
          // make it look jagged rather than a solid line
          if (ec <= 2 || (ec >= 6 && ec <= 8) || (ec >= 11 && ec <= 12)) {
            return true;
          }
        }
      }

      // 9. Asymmetric Battle Scar on the left cheek (adds massive demonic personality)
      if (c < 50) {
        const expectedR = 34 + (c - 36) * 2.2;
        if (c >= 35 && c <= 46 && Math.abs(r - expectedR) <= 1.0) {
          return true;
        }
      }
    ...
    // Create list of empty cells for each turtle
    const turtleCells: { r: number; c: number; dist: number }[][] = Array.from({ length: N }, () => []);

    for (let r = 0; r < 100; r++) {
      for (let c = 0; c < 100; c++) {
        if (isEvilFacePixel(r, c)) continue; // Exclusive face pixels
        let nearestIdx = 0;
        let minDist = Infinity;
        for (let i = 0; i < N; i++) {
          const dy = r - anchors[i].r;
          const dx = c - anchors[i].c;
        ...
        turtleCells[nearestIdx].push({ r, c, dist: minDist });
      }
    }

    // Sort cells for each turtle by distance (ascending) to grow outward from their anchor
    for (let i = 0; i < N; i++) {
      turtleCells[i].sort((a, b) => a.dist - b.dist);
    }

    const gridColors = Array.from({ length: 100 }, () => Array(100).fill('#334155'));

    // Pre-populate actual gray colors for the central ominous face pixels
    for (let r = 0; r < 100; r++) {
      for (let c = 0; c < 100; c++) {
        if (isEvilFacePixel(r, c)) {
          gridColors[r][c] = '#64748b'; // Beautiful distinct ominous grey color
        }
      }
    }

    turtles.forEach((turtle, i) => {
      const cells = turtleCells[i];
      if (!cells || cells.length === 0) return;

      const isNegative = turtle.profit < 0;
      const blueCount = Math.round(isNegative ? Math.max(0, turtle.initialCapital + turtle.profit) : turtle.initialCapital);
      const greenCount = Math.round(!isNegative ? turtle.profit : 0);
      const redCount = Math.round(isNegative ? Math.abs(turtle.profit) : 0);

      const allocatedBlue = Math.min(cells.length, blueCount);
      const allocatedGreen = Math.min(cells.length - allocatedBlue, greenCount);
      const allocatedRed = Math.min(cells.length - allocatedBlue - allocatedGreen, redCount);

      for (let j = 0; j < cells.length; j++) {
        const cell = cells[j];
        if (j < allocatedBlue) {
          gridColors[cell.r][cell.c] = '#3b82f6'; // Bright vibrant Blue - Invested Capital
        } else if (j < allocatedBlue + allocatedGreen) {
          gridColors[cell.r][cell.c] = '#10b981'; // Bright vibrant Green - Profit
        } else if (j < allocatedBlue + allocatedGreen + allocatedRed) {
          gridColors[cell.r][cell.c] = '#ef4444'; // Bright vibrant Red - Loss/Minus
        } else {
          gridColors[cell.r][cell.c] = '#334155'; // Dark slate empty background
        }
      }
    });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {/* Wizualizacja Kuli Śnieżnej 10 000 zł - zajmująca całą szerokość, bez legendy i opisów z ciemnym tłem i etykietami żółwi na obwodzie */}
      <div className="bg-slate-950 p-12 sm:p-14 md:p-16 rounded-3xl shadow-2xl border border-slate-900 overflow-visible relative flex flex-col items-center justify-center">
        <div ref={containerRef} className="w-full max-w-[600px] flex justify-center items-center relative py-6 overflow-visible">
          <div className="relative" style={{ width: `${size}px`, height: `${size}px` }}>
            <canvas
              ref={canvasRef}
              className="block rounded-lg shadow-2xl shadow-black/80"
              style={{ imageRendering: 'pixelated', width: `${size}px`, height: `${size}px` }}
            />
            {/* Etykiety i ikony żółwi na obwodzie kwadratu usunięte */}
          </div>
        </div>
      </div>

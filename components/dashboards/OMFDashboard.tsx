...
import { TrendingUp, Wallet, Timer, Activity, Calendar, Milestone, LayoutTemplate, ScatterChart, CalendarDays, Snowflake, PieChart, ChevronDown, ChevronUp, ArrowUpRight, ArrowDownRight } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className={`w-full p-6 lg:p-8 ${styles.cardContainer} relative overflow-hidden`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 relative z-10">
          ...
          <div className="lg:col-span-4 grid grid-cols-2 gap-3">
             {/* Metric Block: CAGR */}
             <div className={`p-3 rounded-lg border ${isNeon ? 'bg-black/40 border-cyan-900/30' : 'bg-slate-50 border-slate-100'}`}>
      ...
      <div className={`${styles.cardContainer} p-6`}>
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 space-y-4 md:space-y-0">
          ...
          <div className={`flex items-center space-x-3 p-2 rounded-lg border ${isNeon ? 'bg-black/50 border-cyan-900/50' : 'bg-slate-50 border-slate-100'}`}>
              <button onClick={() => setExcludePPK(!excludePPK)} disabled={showCPI || showProjection} className={`flex items-center justify-center w-20 px-2 py-1.5 rounded-md transition-all ${excludePPK ? styles.toggleNoPPKActive : `bg-transparent ${isNeon ? 'text-cyan-700 border-cyan-900/30 hover:text-cyan-400 hover:border-cyan-700' : 'text-slate-500 hover:text-slate-700 border-slate-200'} border`} ${showCPI || showProjection ? 'opacity-50 cursor-not-allowed' : ''}`} title={excludePPK ? "Pokaż PPK" : "Ukryj PPK"}>
                <NoPPKIcon className="w-full h-4" />
              </button>
              <button onClick={() => setShowCPI(!showCPI)} disabled={excludePPK} className={`flex items-center px-3 py-1.5 text-xs font-bold rounded-md transition-all ${showCPI ? styles.toggleCPIActive : `bg-transparent ${isNeon ? 'text-cyan-700 border-cyan-900/30 hover:text-cyan-400 hover:border-cyan-700' : 'text-slate-500 hover:text-slate-700 border-slate-200'} border`} ${excludePPK ? 'opacity-50 cursor-not-allowed' : ''}`}>CPI</button>
              <div className={`w-px h-6 mx-1 ${isNeon ? 'bg-cyan-900/50' : 'bg-slate-200'}`}></div>
              <button onClick={() => setShowProjection(!showProjection)} disabled={excludePPK} className={`flex items-center px-3 py-1.5 text-xs font-bold rounded-md transition-all ${showProjection ? styles.toggleProjectionActive : `bg-transparent ${isNeon ? 'text-cyan-700 border-cyan-900/30 hover:text-cyan-400 hover:border-cyan-700' : 'text-slate-500 hover:text-slate-700 border-slate-200'} border`} ${excludePPK ? 'opacity-50 cursor-not-allowed' : ''}`}><Milestone size={14} className="mr-2" />Droga do Miliona</button>
              {showProjection && (
                <div className="flex items-center space-x-2 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className={`flex rounded-md border p-0.5 ${isNeon ? 'bg-black border-cyan-900/50' : 'bg-white border-slate-200'}`}>
                    <button onClick={() => setProjectionMethod('LTM')} className={`px-2 py-1 text-[10px] font-medium rounded ${projectionMethod === 'LTM' ? (isNeon ? 'bg-cyan-900 text-cyan-300' : 'bg-slate-800 text-white') : (isNeon ? 'text-cyan-700 hover:text-cyan-400' : 'text-slate-500 hover:bg-slate-100')}`}>LTM</button>
                    <button onClick={() => setProjectionMethod('CAGR')} className={`px-2 py-1 text-[10px] font-medium rounded ${projectionMethod === 'CAGR' ? (isNeon ? 'bg-cyan-900 text-cyan-300' : 'bg-slate-800 text-white') : (isNeon ? 'text-cyan-700 hover:text-cyan-400' : 'text-slate-500 hover:bg-slate-100')}`}>CAGR</button>
                  </div>
      ...
      <div className={`${styles.cardContainer} p-6`}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex space-x-2 items-center">
             <div className={`flex items-center space-x-1 p-1 rounded-lg border ${isNeon ? 'bg-black/50 border-cyan-900/50' : 'bg-slate-50 border-slate-100'}`}>
                <button onClick={() => setBubbleChartFilter('ALL')} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${bubbleChartFilter === 'ALL' ? (isNeon ? 'bg-cyan-900 text-cyan-300' : 'bg-white text-slate-800 shadow-sm') : (isNeon ? 'text-cyan-700 hover:text-cyan-400' : 'text-slate-500 hover:bg-slate-100')}`}>Wszystkie</button>
                <button onClick={() => setBubbleChartFilter('KRYPTO')} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${bubbleChartFilter === 'KRYPTO' ? (isNeon ? 'bg-cyan-900 text-cyan-300' : 'bg-white text-slate-800 shadow-sm') : (isNeon ? 'text-cyan-700 hover:text-cyan-400' : 'text-slate-500 hover:bg-slate-100')}`}>Krypto</button>
                <button onClick={() => setBubbleChartFilter('IKE')} className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${bubbleChartFilter === 'IKE' ? (isNeon ? 'bg-cyan-900 text-cyan-300' : 'bg-white text-slate-800 shadow-sm') : (isNeon ? 'text-cyan-700 hover:text-cyan-400' : 'text-slate-500 hover:bg-slate-100')}`}>IKE</button>
             </div>
      ...
      <div className={`${styles.cardContainer} overflow-hidden`}>
        <div className={`px-6 py-4 border-b flex justify-between items-center cursor-pointer transition-colors ${isNeon ? 'bg-black/20 border-cyan-900/30 hover:bg-cyan-950/10' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`} onClick={() => setIsActivePositionsExpanded(!isActivePositionsExpanded)}>
          ...
          <span className={`text-xs font-medium px-2 py-1 rounded-full ${isNeon ? 'bg-emerald-900/40 text-emerald-400 border border-emerald-700/50' : 'bg-emerald-100 text-emerald-700'}`}>{activeAssets.length} pozycji</span>
        </div>
      ...
      <div className={`${styles.cardContainer} overflow-hidden`}>
        <div className={`px-6 py-4 border-b flex justify-between items-center cursor-pointer transition-colors ${isNeon ? 'bg-black/20 border-cyan-900/30 hover:bg-cyan-950/10' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`} onClick={() => setIsClosedHistoryExpanded(!isClosedHistoryExpanded)}>
          ...
          <span className={`text-xs font-medium px-2 py-1 rounded-full ${isNeon ? 'bg-slate-800 text-slate-400 border border-slate-700' : 'bg-slate-200 text-slate-600'}`}>{closedAssets.length} pozycji</span>
        </div>
